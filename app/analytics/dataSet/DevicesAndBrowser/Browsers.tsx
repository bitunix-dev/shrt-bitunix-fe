...
import { ApiResponse, isPaginatedResponse, BrowserData } from "@/app/Get/dataTypes";
import React, { useState, useEffect } from "react";

// Pagination component (identical to Cities component)
const Pagination = ({
  currentPage,
  lastPage,
  onPageChange
...
export const Browsers: React.FC<BrowserProps> = ({
  data,
  setData,
  isClickShortLink
}) => {
  const { data: initialData, isLoading: initialLoading } = useGetClicksBrowsers();
      ...
      if (initialData?.data) {
        ...
        if (!isPaginatedResponse(initialData.data)) {
          ...
          setData({
            currentPage: 1,
            lastPage: 1,
            data: initialData.data as BrowserData[],
          ...
          setData({
            ...
            data: initialData.data.data as BrowserData[],
  ...
  const fetchData = async (page: number) => {
    setLoading(true);
    try {
      const response = await clientApiRequest<ApiResponse<BrowserData>>({
        endpoint: "analytics/browsers",
        method: "GET",
        params: { page }
      });
      ...
      console.error("Error fetching paginated browser data:", error);
    } finally {
      setLoading(false);
    }
  };
