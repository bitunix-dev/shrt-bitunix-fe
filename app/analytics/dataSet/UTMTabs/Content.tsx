...
export const Content:React.FC<ContentProps> = ({
  ...
  const fetchData = async (page: number) => {
    setLoading(true);
    try {
      const response = await clientApiRequest<ApiResponse<ContentData>>({
      ...
      console.error("Error fetching paginated content data:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      {isLoading ? (
          ...
          {data.data.map((item, index) => (
            ...
            >
              <div className="flex items-center">
                {icons.content}
                <p>{item.content || "Unknown Content"}</p>
              </div>
        ...
        <p className="text-center text-gray-500">No content data available</p>
      )}
    </div>
  );
};