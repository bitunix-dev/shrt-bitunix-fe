...
import { ApiResponse, isPaginatedResponse, TermData } from "@/app/Get/dataTypes";
...
export const Term: React.FC<TermProps> = ({
  ...
  const { data: initialData, isLoading: initialLoading } = useGetClicksTerm();
      ...
      if (initialData?.data) {
        ...
        if (!isPaginatedResponse(initialData.data)) {
          ...
          setData({
            currentPage: 1,
            lastPage: 1,
            data: initialData.data as TermData[],
          ...
          setData({
            ...
            data: initialData.data.data as TermData[],
  ...
  const fetchData = async (page: number) => {
    setLoading(true);
    try {
      const response = await clientApiRequest<ApiResponse<TermData>>({
        endpoint: "analytics/terms",
        method: "GET",
        params: { page }
      });
      ...
      console.error("Error fetching paginated term data:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      {isLoading ? (
        ...
        <p className="text-center text-gray-500">No term data available</p>
      )}
    </div>
  );
};