...
import { ApiResponse, isPaginatedResponse, MediumData } from "@/app/Get/dataTypes";
...
export const Medium: React.FC<MediumProps> = ({
  data,
  setData,
  isClickShortLink
}) => {
  const { data: initialData, isLoading: initialLoading } = useGetClicksMedium();
      ...
      if (initialData?.data) {
        ...
        if (!isPaginatedResponse(initialData.data)) {
          ...
          setData({
            currentPage: 1,
            lastPage: 1,
            data: initialData.data as MediumData[],
          ...
          setData({
            ...
            data: initialData.data.data as MediumData[],
  ...
  const fetchData = async (page: number) => {
    setLoading(true);
    try {
      const response = await clientApiRequest<ApiResponse<MediumData>>({
      ...
      console.error("Error fetching paginated medium data:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      {isLoading ? (
          ...
          {data.data.map((item, index) => (
            ...
            >
              <div className="flex items-center">
                {icons.medium}
                <p>{item.medium || "Unknown Medium"}</p>
              </div>
        ...
        <p className="text-center text-gray-500">No medium data available</p>
      )}
    </div>
  );
};