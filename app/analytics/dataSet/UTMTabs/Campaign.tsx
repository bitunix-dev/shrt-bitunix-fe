...
import { ApiResponse, isPaginatedResponse, CampaignData } from "@/app/Get/dataTypes";
...
export const Campaign: React.FC<CampaignProps> = ({
  ...
  const { data: initialData, isLoading: initialLoading } = useGetClicksCampaign();
      ...
      if (initialData?.data) {
        ...
        if (!isPaginatedResponse(initialData.data)) {
          ...
          setData({
            currentPage: 1,
            lastPage: 1,
            data: initialData.data as CampaignData[],
          ...
          setData({
            ...
            data: initialData.data.data as CampaignData[],
  ...
  const fetchData = async (page: number) => {
    setLoading(true);
    try {
      const response = await clientApiRequest<ApiResponse<CampaignData>>({
      ...
      {isLoading ? (
        ...
        <>
          {data.data.map((item, index) => (
            ...
            >
              <p>{item.campaign || "Unknown Campaign"}</p>
